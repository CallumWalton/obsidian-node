...
function Get-CncConfiguration {
    ...
    Write-ColorOutput "`n🚀 Obsidian Command and Control Cloud-Init Generator" -ForegroundColor "Magenta"
    ...
    $config.DOMAIN = Read-ValidatedInput `
        -Prompt "Enter your CNC server domain (e.g. cnc.obsidian.example.com)" `
...
function New-CredentialsSummary {
    ...
    $summary = @"
Obsidian Command and Control - Generated Credentials
=================================================
Generated: $timestamp
Domain: $($Config.DOMAIN)
Admin Email: $($Config.ADMIN_EMAIL)
...
function Main {
    try {
        Write-ColorOutput "`n🔧 Obsidian CNC Cloud-Init Generator" -ForegroundColor "Magenta"
        Write-ColorOutput "=" * 60 -ForegroundColor "Gray"
        Write-Info "This script will generate a complete cloud-init configuration for deploying"
        Write-Info "the Obsidian Command and Control server on Hetzner Cloud or other providers."
        Write-ColorOutput ""
        ...
        Write-ColorOutput "  Config JSON: $configJsonFile" -ForegroundColor "White"
        ...
        Write-Warning "`nIMPORTANT: Keep the credentials file secure!"
        Write-Warning "The generated passwords are required for initial system access."
...
"@
        Write-ColorOutput $deploymentExample -ForegroundColor "Gray"
